...
<%  //1. utf-8 설정
	...
	try{
		boolean again=true;
		while(again){  //#1-0  입력을  계속시도.
			String sql0 = "select max(ono) `max` from milk_order";  //#1-1 제일큰 번호 - 마지막에 입력된 번호
			String sql  = "insert into  milk_order (ono, oname, onum, oip)  values (?  , ? , ?,  ?);  ";  //#1-2 직접 번호 입력방법으로 전환
			int max=0;
			...
			//#1-4  ..................... insert에 성공하면 
			if(result>0){ 
				again=false;  //#1-5 다시하지마
				out.println( 
					"<script>alert('주문성공! 주문번호는 " + max + " 입니다.'); location.href='milk.jsp'; </script>" ); 
				// 주문번호 출력	
			}
